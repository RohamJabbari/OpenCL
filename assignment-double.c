...
void sequential(double** mainMatrix, double** secondaryMatrix, int size)
{
    double* holder;
...
int main(int argc, char* argv[]){
    ...
    //buffer size
    int bufferSize = SIZE * SIZE * sizeof(double);
    ...
    seqMat = (double *)malloc(bufferSize);
    tempSeqMat = (double *)malloc(bufferSize);
    ...
    S_TIME = (double)(S_END-S_START) / CLOCKS_PER_SEC;
...
//----------------------------------------------------
    ...
    //parallel matrices
    double* mainMatrix;
    double* secondaryMatrix;
    
    mainMatrix = (double *)malloc(bufferSize);
    secondaryMatrix = (double *)malloc(bufferSize);
    ...
    kernelFile = fopen("assignment-double.cl", "r");
    ...
    P_TIME = (double)(P_END-P_START) / CLOCKS_PER_SEC;
...
// test
    int err_count = 0;
    // // double e = 2.225E-307; // smallest positive double value
    // double e = FLT_MIN;
    for (int i = 0; i < SIZE; i++) {
		for (int j = 0; j < SIZE; j++) {            
